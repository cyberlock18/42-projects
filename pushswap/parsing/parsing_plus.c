...
/*   Updated: 2025/01/08 09:24:21 by ruortiz-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void free_split(char **args)
{
    ...
    if (!args) // Verificar que args no sea NULL
        return;
    i = 0;
    while (args[i])
    {
        free(args[i]);
        args[i] = NULL; // Asignar NULL después de liberar
        i++;
    }
    free(args);
    args = NULL; // Asignar NULL al puntero principal
}
...
void handle_error(t_node **stack, char *trim)
{
    ...
    if (stack && *stack)
        free_stack(stack); // Verificar que stack no haya sido liberado previamente
    ft_printf("Error\n");
    exit(1);
}
